// src/pages/setup/pathology/PathologySetupPage.tsx
import { useEffect, useState } from 'react'
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query'
import { Plus, Search, Edit2, Trash2, X } from 'lucide-react'
import { pathSetupApi } from '@/api/pathologySetup'
import { cn } from '@/lib/utils'

type Tab = 'category' | 'unit' | 'parameter'

const TABS: Array<{ id: Tab; label: string }> = [
  { id: 'category',  label: 'Pathology Category' },
  { id: 'unit',      label: 'Unit' },
  { id: 'parameter', label: 'Pathology Parameter' },
]
...
function CategoryTab() {
  return <SimpleTab
    listTitle="Pathology Category List"
    singular="Pathology Category"
    columnHeader="Category Name"
    listFn={() => pathSetupApi.listCategories().then(r => r.data)}
    addFn={(d) => pathSetupApi.addCategory(d)}
    updateFn={(id, d) => pathSetupApi.updateCategory(id, d)}
    deleteFn={(id) => pathSetupApi.deleteCategory(id)}
    queryKey={['path-cats']}
  />
}
...
function UnitTab() {
  return <SimpleTab
    ...
    listFn={() => pathSetupApi.listUnits().then(r => r.data)}
    addFn={(d) => pathSetupApi.addUnit(d)}
    updateFn={(id, d) => pathSetupApi.updateUnit(id, d)}
    deleteFn={(id) => pathSetupApi.deleteUnit(id)}
    queryKey={['path-units']}
  />
}
...
function ParameterTab() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn:  () => pathSetupApi.listParameters().then(r => r.data),
  })
  const items: any[] = data?.data ?? []
  const filtered = search
    ? items.filter(r =>
  ...
  const del = useMutation({
    mutationFn: (id: number) => pathSetupApi.deleteParameter(id),
    onSuccess:  () => qc.invalidateQueries({ queryKey: ['path-params'] }),
  })

  return (
    <div className="card">
      <div className="flex items-center justify-between px-5 py-3 border-b">
        <h2 className="text-base font-semibold text-gray-800">Pathology Parameter List</h2>
        <button onClick={() => setModal({ open: true })}
          className="btn btn-primary flex items-center gap-1.5">
          <Plus size={14}/> Add Pathology Parameter
        </button>
      </div>
      <div className="px-5 py-3 border-b">
        <div className="relative max-w-xs">
...
function ParameterModal({ open, onClose, item }: { open: boolean; onClose: () => void; item?: any }) {
  ...
  const { data: unitsData } = useQuery({
    queryKey: ['path-units'],
    queryFn:  () => pathSetupApi.listUnits().then(r => r.data),
    enabled:  open,
  })
  ...
  const save = useMutation({
    mutationFn: (payload: any) => editing
      ? pathSetupApi.updateParameter(item.id, payload)
      : pathSetupApi.addParameter(payload),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['path-params'] })
      onClose()
    },
  })
  ...
  return (
    <Modal title={editing ? 'Edit Pathology Parameter' : 'Add Pathology Parameter'} onClose={onClose}>
      <form onSubmit={(e) => {
        e.preventDefault()
