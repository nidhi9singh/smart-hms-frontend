            ...
            <tbody>
              {isLoading ? (
                ...
                <tr key={t.id} className="border-t hover:bg-gray-50">
                  <td className="px-3 py-2 font-medium text-brand-700">{t.name}</td>
                  <td className="px-3 py-2 text-gray-600">
                    {variant === 'certificate' ? '—' : (
                      t.background_image ? (
                        <FileImage size={20} className="text-brand-500"/>
                      ) : '—'
                    )}
                  </td>
                  <td className="px-3 py-2 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button onClick={() => setEdit({ open: true, template: t })} className="p-1 hover:bg-brand-50 rounded text-brand-600"><Edit2 size={14}/></button>
