                      ...
                      {l.status === 'Pending' && (
                        <>
                          <button onClick={() => quickApprove(l.id)} title="Approve"
                            className="p-1.5 text-green-500 hover:bg-green-50 rounded border border-gray-200">
              ...
              {STATUS_OPTIONS.map(s => (
                <label key={s} className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="leave_status" value={s} checked={form.status === s}
                    onChange={() => setField('status', s)} className="w-4 h-4 text-blue-500 border-gray-300 focus:ring-blue-400" />
